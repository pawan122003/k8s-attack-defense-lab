...
func (s *Server) HandleScript(scriptRelPath string, logger *log.Logger, f func(http.Handler)) error {
	cleanRelPath := filepath.Clean(scriptRelPath)
	if strings.HasPrefix(cleanRelPath, "..") || strings.Contains(cleanRelPath, string(os.PathSeparator)+"..") || strings.HasPrefix(cleanRelPath, string(os.PathSeparator)) {
		return fmt.Errorf("script path traversal attempt detected")
	}
	ri, ok := s.scriptCache.Load(cleanRelPath)
	if !ok {
		ri, _ = s.scriptCache.LoadOrStore(cleanRelPath, new(scriptResult))
	}
	r := ri.(*scriptResult)

	relDir := strings.TrimSuffix(cleanRelPath, filepath.Ext(cleanRelPath))
	...
	for {
		...
		content, err := os.ReadFile(filepath.Join(s.scriptDir, cleanRelPath))
		if err != nil {
			if !os.IsNotExist(err) {
				return err
			}
		...
		if prevHash := r.hash; prevHash != hash {
			...
			func() {
				...
				scriptHandler, err := s.loadScript(context.Background(), logger, cleanRelPath, content, workDir)
				if err != nil {
					r.err = err
					return
				}
