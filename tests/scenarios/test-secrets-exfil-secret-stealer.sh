...
# Test: Secrets Exfiltration - Secret Stealer
set -e

TEST_NAME="secrets-exfil-secret-stealer"
NAMESPACE="test-${TEST_NAME}"
ATTACK_FILE="attacks/secrets-exfil/secret-stealer.yaml"
...
verify_attack() {
    ...
    if [ "$pods" -gt 0 ]; then
        echo "PASS: Secret stealer pod deployed successfully"
        return 0
    fi
