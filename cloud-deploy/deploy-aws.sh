...
echo "🏗️ Deploying K8s Attack-Defense Lab to AWS EKS"
echo "=============================================="
...
if [ -z "$AWS_REGION" ]; then
    echo "❌ AWS_REGION environment variable must be set"
    echo "Example: export AWS_REGION=us-east-1"
    exit 1
fi

# Navigate to AWS deployment directory
echo "📂 Changing to AWS deployment directory..."
cd "$ORIGINAL_DIR/cloud-deploy/aws" || {
    echo "❌ Failed to change to AWS deployment directory"
    exit 1
}
...
if [ -f "cluster/kind-config.yaml" ]; then
    echo "⚠️  Note: Deploying Kind config to EKS (may need adjustments)"
